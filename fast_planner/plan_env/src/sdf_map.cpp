...
void SDFMap::initMap(MappingParameters& mp) {
  ...
  cout << "[sdfmap]: hit: " << mp_.prob_hit_log_ << endl;
  cout << "[sdfmap]: miss: " << mp_.prob_miss_log_ << endl;
  cout << "[sdfmap]: min log: " << mp_.clamp_min_log_ << endl;
  cout << "[sdfmap]: max: " << mp_.clamp_max_log_ << endl;
  cout << "[sdfmap]: thresh log: " << mp_.min_occupancy_log_ << endl;
...
void SDFMap::depthOdomCallback(const sensor_msgs::msg::Image::ConstSharedPtr img, const nav_msgs::msg::Odometry::ConstSharedPtr odom){
  ...
  Eigen::Matrix3d camera_R = robot_R * camera_body_r;
